...
class ChannelManager {
    ...
    fun init() {
        Log.d("POTATO", "ChannelManager:init on threadID = ${Thread.currentThread().id}")
        joinedChannels = ChannelRepository.instance?.getJoinedChannels(AccountRepository.getAccount().blockingGet().sessionToken)?.blockingGet()!!
        availableChannels = ChannelRepository.instance?.getAvailableChannels(AccountRepository.getAccount().blockingGet().sessionToken)?.blockingGet()!!
    ...
    fun onChannelCreated(channel: Channel) {
        if (channel.users.get(0).ID == AccountRepository.getAccount().blockingGet().ID) {
